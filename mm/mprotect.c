 ...
 * pkey==-1 when doing a legacy mprotect()
 */
static int remote_do_mprotect_pkey(struct task_struct *child, unsigned long start, size_t len,
							unsigned long prot, int pkey)
{
	unsigned long nstart, end, tmp, reqprot;
	struct vm_area_struct *vma, *prev;
	int error = -EINVAL;
	const int grows = prot & (PROT_GROWSDOWN|PROT_GROWSUP);
	const bool rier = (child->personality & READ_IMPLIES_EXEC) &&
					  (prot & PROT_READ);
	 ...
	 */
	error = -EINVAL;
	if ((pkey != -1) && !mm_pkey_is_allocated(child->mm, pkey))
		goto out;
